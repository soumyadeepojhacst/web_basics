...
export class hostelService {
    ...
    updateResident(residentId, updatedData) {
        ...
        if (updatedData.roomNumber !== undefined &&
            ...
            // Free old room
            const oldRoom = this.rooms.find(r => Number(r.roomNumber) === Number(existingResident.roomNumber));
            if (oldRoom) {
                oldRoom.isOccupied = false;
            }
            // Check new room
            const newRoom = this.rooms.find(r => Number(r.roomNumber) === Number(updatedData.roomNumber));
            if (!newRoom) {
        ...
        this.resident[index] = Object.assign(Object.assign({}, existingResident), updatedData);
        //  Save everything
        this.saveData();
